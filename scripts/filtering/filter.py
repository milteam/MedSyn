...
def bert_metrics(df: pd.DataFrame, left_column: str, right_column: str):
    return score(
        df[left_column].tolist(),
        df[right_column].str.replace("|", ",", regex=False).tolist(),
        lang="ru",
    )
...
@click.option(
    "--input-filepath",
    "-i",
    help="A path to an input JSONL file with model responses.",
    required=True,
)
...
def main(
    ...
    print(
        f"\033[34mAnamnesis precision threshold = {anamnesis_precision_threshold:.2f}\033[0m"
    )
    ...
    df["FILTERING_OK"] = (df["symptoms_recall"] >= symptoms_recall_threshold) & (
        df["anamnesis_precision"] > anamnesis_precision_threshold
    )
