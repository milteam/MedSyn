...
with open("data/train_v1.jsonl") as f:
    for row in f.readlines():
        row = json.loads(row)
        sent1 = row["ru_sentence1"].strip()
        sent2 = row["ru_sentence2"].strip()
...
logging.info("Read RuMEdBench dev dataset")
dev_samples = []
with open("data/dev_v1.jsonl") as f:
    for row in f.readlines():
        ...
        dev_samples.append(
            InputExample(
                texts=[row["ru_sentence1"], row["ru_sentence2"]],
                label=LABELS[row["gold_label"]],
            )
        )
...
test_evaluator = EmbeddingSimilarityEvaluator.from_input_examples(
    dev_samples, batch_size=train_batch_size, name="sts-test"
)
