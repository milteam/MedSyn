...
class InstructDataset(Dataset):
    ...
    def convert_record(self, record):
        ...
        if inp.strip() != "":
            templates = self.template["prompts_input"]
            #print('templates', templates)
            prompt_template = random.choice(templates)
            source = prompt_template.format(instruction=instruction.strip(), input=inp.strip())
        else:
            templates = self.template["prompts_no_input"]
            #print('templates', templates)
            prompt_template = random.choice(templates)
...
class ChatDataset(Dataset):
    ...
    def convert_record(self, record):
        conversation = Conversation.from_template(self.template_path)
