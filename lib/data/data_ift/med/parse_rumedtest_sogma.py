...
def generate_data(
    ...
    result_name: str = "rumedtest_data_ift.json",
    ...
    for q in tqdm(dd["geetest"]["test"]["questions"]["question"]):
        ...
        for a in q["answers"]["answer"]:
            aa[a["@num"]] = a["#text"]
        correct_idx = [
            a["@num"] for a in q["answers"]["answer"] if a["@isCorrect"] == "1"
        ]
        try:
    ...
    )  # keep only 1-line questions, drop option 6(unused)
